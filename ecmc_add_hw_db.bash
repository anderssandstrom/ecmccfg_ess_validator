...
echo "$ALLFILES " | awk -v pfix=$prefix -v name=$naming ' BEGIN{i=100;} {print "${SCRIPTEXEC}\"${ecmccfg_DIR}/ecmc" name ".cmd\" \"MASTER_ID=0,SLAVE_POS=" i ",HWTYPE=" $1"\""; print "dbLoadTemplate(ecmc" $1 ".substitutions,\"ECMC_P=${ECMC_P},ECMC_G=${ECMC_G},PORT=MC_CPU1,ADDR=0,TIMEOUT=1,MASTER_ID=0,SLAVE_POS=0,HWTYPE=" $1 ",T_SMP_MS=10,TSE=-2,NELM=1\")"; i=i+1;}'
